...
console.log("allData",allData);
  ...
  return (
    ...
    <div>
      ...
      <div>
        ...
        <div className="min-h-[70vh] grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4   max-w-6xl mx-auto p-3">
          {men.map((men) => (
            <div
              key={men.id}
              className="group flex flex-col items-center border-2 border-red-800 gap-3 p-4 h-[360px] mt-2 ml-5 rounded-xl"
            >
              <div className="h-[180px]">
                ...
                <div className="flex items-center justify-center w-full mt-2">
                  <button
                    onClick={
                      cart.some((item) => item.id === men.id)
                        ? () => dispatch(removeFromCart(men.id))
            ...
            >
              <div className="h-[180px]">
                ...
                <div className="flex items-center justify-center w-full mt-2">
                  <button
                    onClick={
                      cart.some((item) => item.id === women.id)
                        ? () => dispatch(removeFromCart(women.id))
                        : () => dispatch(addToCart(women))
                    }
                    className="bg-red-950 text-white border-2 rounded-lg p-2"
                  >
                    {cart.some((item) => item.id === women.id)
      ...
      {/* <!-- jewelery --> */}
      <div>
        <h1 className="font-semibold text-2xl mt-5  ml-28 text-gray-800">
          Jewelery
        </h1>
        <div className="min-h-[70vh] grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4  max-w-6xl mx-auto p-3">
              ...
              <div className="h-[180px]">
                ...
                <div className="flex flex-col items-center justify-center">
                  <div className="flex items-center justify-around">
                    <div className="flex items-center font-semibold bg-green-700 text-white rounded-md p-0.5">
                      <p>
                        <span>{jewelery.rating.rate}</span>
                      </p>
                      <IoStarSharp />
                    </div>
                    <p className="text-md font-extralight">
                      {jewelery.rating.count}
                    </p>
                  </div>
                  <p className="font-semibold text-lg text-gray-600">
                    ${jewelery.price}
                ...
                <div className="flex items-center justify-center w-full mt-2">
                  <button
                    onClick={
                      cart.some((item) => item.id === jewelery.id)
                        ? () => dispatch(removeFromCart(jewelery.id))
                        : () => dispatch(addToCart(jewelery))
                    }
                    className="bg-red-950 text-white border-2 rounded-lg p-2"
                  >
                    {cart.some((item) => item.id === jewelery.id)
              ...
              <div className="h-[180px]">
                ...
                <div className="flex flex-col items-center justify-center">
                  <div className="flex items-center justify-around">
                    <div className="flex items-center font-semibold bg-green-700 text-white rounded-md p-0.5">
                      <p>
                        <span>{electonic.rating.rate}</span>
                      </p>
                      <IoStarSharp />
                    </div>
                    <p className="text-md font-extralight">
                      {electonic.rating.count}
                    </p>
                  </div>
                  <p className="font-semibold text-lg text-gray-600">
                    ${electonic.price}
                ...
                <div className="flex items-center justify-center w-full mt-2">
                  <button
                    onClick={
                      cart.some((item) => item.id === electonic.id)
                        ? () => dispatch(removeFromCart(electonic.id))
                        : () => dispatch(addToCart(electonic))
                    }
                    className="bg-red-950 text-white border-2 rounded-lg p-2"
                  >
                    {cart.some((item) => item.id === electonic.id)
